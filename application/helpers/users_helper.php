...
function get_permission($menu, $uid = NULL, $ugroup = NULL)
{
  $CI =& get_instance();
  ...
  {
    $pm = $CI->user_model->get_permission($menu, $user->ugroup);
...
function select_profile($ugroup = '')
{
  $ds = '';
  $CI =& get_instance();
  ...
  if(!empty($qs))
  {
    foreach($qs as $rs)
    {
      $ds .= '<option value="'.$rs->id.'" '.is_selected($rs->id, $ugroup).'>'.$rs->name.'</option>';
    }
  }
